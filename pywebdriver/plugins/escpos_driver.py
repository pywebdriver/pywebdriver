...
class ESCPOSDriver(ThreadDriver, POSDriver):
    ...
    def connected_usb_devices(self):
        ...
        for device in SUPPORTED_DEVICES:
            if (
                usb.core.find(idVendor=device["vendor"], idProduct=device["product"])
                is not None
            ):
        ...
        try:
            if device_type == "usb":
                ...
                if printers:
                    ...
                    self.open()
                    self.vendor_product = "{}_{}".format(self.idVendor, self.idProduct)
    ...
    # #####################################################################
    # <Odoo Version 7>
    def print_receipt_7(self, receipt):  # noqa: C901
        def check(string):
        ...
        if not self.eprint:
            printers = self.connected_usb_devices()
            if len(printers) > 0:
                self.eprint = POSDriver(printers[0]["vendor"], printers[0]["product"])
            else:
                return
        ...
        if money(receipt["subtotal"]) != money(receipt["total_with_tax"]):
            ...
            print_taxes()
            eprint.text(
                printline(_(u"Taxes"), money(receipt["total_tax"]), width=40, ratio=0.6)
            )
            taxincluded = False
